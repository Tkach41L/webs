		...
		.then((resJSON) => {
			...
			if (opinionsFromStorage) {
				...
				opinions.forEach((opinion) => {
					opinion.created = new Date(opinion.createdAt).toDateString();
				...
				fetch(`${url}/comment`)
					...
					.then((commentsJSON) => {
						...
						if (auth2.isSignedIn.get()) {
							document.getElementById('comment_author').value = auth2.currentUser
								.get()
								.getBasicProfile()
								.getName(); //if signin to add v comment author user name
						}
...
function addComment(artIdFromHash) {
	const commentAuthor = document.getElementById('comment_author').value.trim();
